...
def open_file(file):
    ...
    '''
    sorted_Total_tongjis = sorted(Total_tongjis)
...
统计结果如下所示：
统计中位数是：{np.percentile(sorted_Total_tongjis, 50)}
统计第一三分位数：{np.percentile(sorted_Total_tongjis, 33.33)}
统计第二三分位数：{np.percentile(sorted_Total_tongjis, 66.67)}
统计第一四分位数：{np.percentile(sorted_Total_tongjis, 25)}
统计第3四分位数：{np.percentile(sorted_Total_tongjis, 75)}
统计40分位数：{np.percentile(sorted_Total_tongjis, 40)}
""")
