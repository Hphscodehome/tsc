...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            counter = 0  # 重置计数器
            torch.save(model.state_dict(), f'/data/hupenghui/Self/tsc/ticket/model2/best_modelv2_{args.end}.pth') # 保存当前最好的模型
            ...
            if counter >= patience:
                ...
                break  # 提前结束训练循环
    logging.info(f"当前试验截断值为：{args.end},文件名为：/data/hupenghui/Self/tsc/ticket/model2/best_modelv2_{args.end}.pth,停止epoch:{epoch-counter},最优模型验证损失为：{best_val_loss}")
    with open('/data/hupenghui/Self/tsc/ticket/record.txt', 'a', encoding='utf-8') as file:
        file.write(f"当前试验截断值为：{args.end},文件名为：/data/hupenghui/Self/tsc/ticket/model2/best_modelv2_{args.end}.pth,停止epoch:{epoch-counter},最优模型验证损失为：{best_val_loss}\n")